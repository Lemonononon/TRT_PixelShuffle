...
__global__ void PixelShuffleKernel(const float* input, float* output, int batchSize, int channels, int height, int width, int upscaleFactor)
{
    ...
    int in_c = out_c * upscaleFactor * upscaleFactor + (out_h % upscaleFactor) * upscaleFactor + (out_w % upscaleFactor);
...
int32_t PixelShufflePlugin::enqueue(nvinfer1::PluginTensorDesc const* inputDesc, nvinfer1::PluginTensorDesc const* outputDesc,
void const* const* inputs, void* const* outputs, void* workspace, cudaStream_t stream) noexcept{
    ...
    PixelShuffleKernel<<<(numElements + 255) / 256, 256>>>(input, output, batchSize, channels, height, width, upscaleFactor);
